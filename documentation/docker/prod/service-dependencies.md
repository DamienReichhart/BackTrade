...
graph TB
    subgraph "Startup Sequence"
        ...
        subgraph "Phase 1: Infrastructure Services"
            ...
            MinIO[MinIO<br/>Health Check: curl /health/live<br/>Start Period: 30s]
        ...
        subgraph "Phase 3: Application Services"
            Backend[Backend API<br/>Depends on: Migrate, Postgres, Redis,<br/>RabbitMQ, MinIO<br/>Condition: service_completed_successfully<br/>for Migrate, service_healthy for others<br/>Health Check: HTTP /api/v1/health<br/>Start Period: 40s]

            Worker[Worker Service<br/>Depends on: Migrate, Postgres, ClickHouse,<br/>RabbitMQ, MinIO<br/>Condition: service_completed_successfully<br/>for Migrate, service_healthy for others]
