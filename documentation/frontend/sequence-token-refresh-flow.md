...
sequenceDiagram
    ...
    participant API
    participant retryIfUnauthenticated
    ...
    alt Request successful
        ...
        executeFetch->>retryIfUnauthenticated: retryIfUnauthenticated(response, refreshToken, retryFunction, onTokenRefresh)

        retryIfUnauthenticated->>retryIfUnauthenticated: Check if refreshToken exists

        alt No refresh token
            retryIfUnauthenticated-->>executeFetch: Throw error
            executeFetch-->>useFetch: Error
            useFetch-->>Component: Error (authentication required)
        else Refresh token exists
            retryIfUnauthenticated->>refreshAccessToken: refreshAccessToken(refreshToken)
            ...
            alt Token refresh successful
                ...
                refreshAccessToken-->>retryIfUnauthenticated: AuthResponse

                retryIfUnauthenticated->>AuthStore: onTokenRefresh(newAccessToken, newRefreshToken)
