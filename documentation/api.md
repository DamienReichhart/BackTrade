...
| **ResetPasswordRequest**      | `{ code: string, newPassword: string }`                                                                                                                                                                                                                                                                                                                                                         | Request body for password reset with code (newPassword min 8 chars) |
...
| **Candle**                    | `{ instrument_id: number, timeframe: Timeframe, ts: string, open: number, high: number, low: number, close: number, volume: number, created_at: string, updated_at: string }`                                                                                                                                                                                                                   | OHLCV Candle entity identified by (instrument_id, timeframe, ts)    |
...
| **SessionAnalyticsResponse** | `{ summary, equity_curve, breakdowns, costs, top_winners, worst_losers, daily_pnl }` | Detailed analytics response for a session |
...
| **PATCH**  | `/candles`                   | `instrument_id`, `timeframe`, `ts` | `UpdateCandleRequest`       | `Candle`                   | 200, 400, 401, 403, 404      | Update candle identified by composite key                     |
| **DELETE** | `/candles`                   | `instrument_id`, `timeframe`, `ts` | -                           | -                          | 204, 401, 403, 404           | Delete candle identified by composite key                     |
